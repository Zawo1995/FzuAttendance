...
import com.framework.websocket.SpringWebSocketHandler;
import com.framework.websocket.WebSocketHandler;
...
public class Common {
     ...
     * @return
     */
    public static List<List> splitArray(List list, int length){
        ...
        if (list.size() < length){
            ...
            for (int i = 0; i * length <= list.size(); i++){
                returnList.add(list.subList(i * length, length));
            }
        }
        return returnList;
    }
