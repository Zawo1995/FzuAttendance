...
;!function(){
    ...
    //触发事件
    LAY.prototype.trigger = function(eventType){
        return this.each(function(index, item){
    ...
    Class.prototype.init = function(){
        ...
        if(options.value && options.isInitValue){
            if(options.value.constructor === Date){
                that.setValue(options.value = that.parse(0, that.systemDate(options.value)));
            } else if (options.value.constructor === Array){
                var valueT = '';
                if(options.value[0] && options.value[0].constructor === Date){
                    valueT = that.parse(1, that.systemDate(options.value[0]));
                }
                if(options.value[1] && options.value[1].constructor === Date){
                    valueT += ' ' + options.range + ' ' + that.parse(1, that.systemDate(options.value[1]));
                }
                that.setValue(options.value = valueT);
            } else {
    ...
    Class.prototype.checkDate = function(fn){
        ...
        // if (value && value.constructor === Array && options.range) value = options.value;
        ...
        } /*else if(value && value.constructor === Array && options.range){
          that.startDate = that.systemDate(value[0]);
          that.endDate = that.systemDate(value[1]);
          options.dateTime = lay.extend({}, that.startDate);
          lay.each([that.startDate, that.endDate], function(i, item){
            initDate(item, value[i], i);
          });
    ...
    Class.prototype.setValue = function(value){
        ...
        options.position === 'static' || lay(elem)[valType](value || '').trigger('input');
        changeEvent(elem);
        return this;
    };
    ...
    Class.prototype.tool = function(btn, type){
        var that = this
            ...
            ,confirm: function(){
                ...
                //  自定义confirm方法，在点击确定时候，done之前使用
                var start = lay.extend({}, that.startDate ? lay.extend(that.startDate, that.startTime) : options.dateTime)
                    ,end = lay.extend({}, lay.extend(that.endDate, that.endTime))
                    ,param = param || [that.parse(), start, end];
                start.month++; end.month++;
                if (typeof options['confirm'] === 'function') {
                    var isConfirm = options['confirm'].apply(options, param);
                    if (typeof isConfirm === 'boolean' && !isConfirm) return;
                }
                //  自定义confirm方法，在点击确定时候，done之前使用 ------end
